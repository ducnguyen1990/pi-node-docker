...
if [[ "$TARGET" == *.well-known/stellar-history.json ]]; then
  ...
  /s3cmd/s3cmd -c /home/stellar/.s3cfg put --acl-public --add-header="Cache-Control":"max-age=0" $SOURCE s3://testnet-history/$TARGET
else
  ...
  # -c option, because for some weird reason, s3cmd looks for the config file in /root although it's running as the stellar user
  /s3cmd/s3cmd -c /home/stellar/.s3cfg put --acl-public $SOURCE s3://testnet-history/$TARGET
fi
